...
- **Dropbox-authenticatie:** wordt omgezet van de handmatige "Access Token"-methode (kortlevend, ~4 uur, moet steeds opnieuw gegenereerd worden) naar **OAuth2** (refresh-token, ververst zichzelf automatisch en verloopt niet). Werkt met dezelfde Dropbox-app (App key/secret hergebruikt als Client ID/Secret), gratis, ook zonder production-approval. Zie roadmap voor status.
...
1. **Verlopen Dropbox-token** crashte de hele workflow-executie stil, ook voor de tekst/voice-branches — Dropbox-nodes hadden geen foutafhandeling, dus één verlopen token trok alles om. Tijdelijk verholpen door het token handmatig te vernieuwen; structurele fix is de omzetting naar OAuth2 (zie roadmap), zodat dit niet meer terugkomt.
...
- [ ] **Dropbox-credential omzetten naar OAuth2.** Lost het steeds-verlopende Access Token structureel op (refresh-token verloopt niet). Zelfde Dropbox-app hergebruiken (App key/secret als Client ID/Secret), nieuwe n8n-credential aanmaken naast de bestaande, Redirect URI koppelen, daarna alle Dropbox-nodes ombouwen naar de nieuwe credential. Let op: de Redirect URI moet exact matchen met de (instabiele) ngrok-URL — zie het punt hieronder.
